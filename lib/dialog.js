...
module.exports = class Dialog {
  constructor(param) {
    ...
    const { initialPath, select, isDirectory, iconClass, prompt, info, checkboxes } = param;
    ...
    if (select) {
      // Select the base name and leave the directory prefix and the extension
      // alone, so a rename types over the name only. The extension is the last
      // one — `spec.js` renames as `spec`, not as `spec.js` minus `.spec.js` —
      // and a leading dot is part of the name, so `.gitignore` selects whole.
      // A directory has no extension to preserve at all.
      ...
      const selectionEnd = initialPath.length - extension.length;
