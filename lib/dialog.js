...
module.exports = class Dialog {
  constructor(param) {
    ...
    this.disposables.add(this.inputDialogHost.onDidCancel(() => this.cancel()));
   ...
   * the new path should open it. `InputDialog#confirm` is that same text
  ...
  confirm({ open = false } = {}) {
    ...
    this.confirmPromise = pending;
    const clearPending = () => {
      if (this.confirmPromise === pending) this.confirmPromise = null;
    };
    pending.then(clearPending, clearPending);

    try {
      Promise.resolve(this.onConfirm(this.miniEditor.getText(), { open })).then(
        resolveConfirm,
        rejectConfirm,
      );
    } catch (error) {
  ...
  attach() {
    this.inputDialogHost.show({ query: this.initialQuery, selectQuery: false });
  ...
  showError(message) {
    this.inputDialog.setStatus(message ? { type: "error", message } : null);
  }
};
