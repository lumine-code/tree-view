...
class TreeView {
  ...
  async resolveFileOperationConflict({ relativePath }) {
    const chosen = await atom.window.confirm({
  ...
  openSelectedEntryInNewWindow() {
    ...
    if (pathToOpen) {
      return atom.app.openWindow({ pathsToOpen: [pathToOpen], newWindow: true });
    }
  }
  ...
  openSelectedEntryInThisWindow() {
    let [pathToOpen] = this.selectedPaths();
    if (pathToOpen) {
      return atom.project.setState([pathToOpen]);
    }
  }
