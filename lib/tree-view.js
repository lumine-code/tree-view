...
class TreeView {
  ...
  moveSelectedEntry() {
    ...
    let dialog = new MoveDialog(oldPath, {
      ...
      willMove: async ({ initialPath, newPath }) => {
        const payload = {
          files: [{ oldPath: initialPath, newPath, isDirectory: fs.isDirectorySync(initialPath) }],
        };
        if (
      ...
      onMove: async ({ initialPath, newPath, open }) => {
        ...
        if (this.fileOperationEvents) {
          await this.fileOperationEvents.did("didRename", {
            files: [{ oldPath: initialPath, newPath, isDirectory: fs.isDirectorySync(newPath) }],
          });
        }
    ...
    let dialog = new CopyDialog(oldPath, {
      ...
      willCopy: ({ initialPath, newPath }) =>
        this.fileOperationEvents?.will("willCreate", {
          paths: [newPath],
          entries: [{ path: newPath, isDirectory: fs.isDirectorySync(initialPath) }],
        }),
      onCopy: async ({ initialPath, newPath, open }) => {
        ...
        if (this.fileOperationEvents)
          await this.fileOperationEvents.did("didCreate", {
            paths: [newPath],
            entries: [{ path: newPath, isDirectory: fs.isDirectorySync(newPath) }],
          });
        await this.revealChangedPath(newPath);
        if (open) lumine.workspace.open(newPath);
      },
  ...
  async removeSelectedEntries() {
    ...
    if (response === 0) {
      ...
      const existingPaths = selectedPaths.filter((selectedPath) => fs.existsSync(selectedPath));
      const existingEntries = existingPaths.map((selectedPath) => ({
        path: selectedPath,
        isDirectory: fs.isDirectorySync(selectedPath),
      }));
      if (
        this.fileOperationEvents &&
      ...
      for (let selectedPath of existingPaths) {
          ...
          .then(async () => {
            ...
            if (this.fileOperationEvents)
              await this.fileOperationEvents.did("didDelete", {
                paths: [selectedPath],
                entries: existingEntries.filter((entry) => entry.path === selectedPath),
              });
          })
          .catch(() => {
    ...
    try {
      if (
        ...
        (await this.fileOperationEvents.will("willCreate", {
          paths: [newPath],
          entries: [{ path: newPath, isDirectory: initialPathIsDirectory }],
        })) === false
      )
        return false;
  ...
  async moveEntry(initialPath, newDirectoryPath) {
    ...
    try {
      const operation = {
        files: [{ oldPath: initialPath, newPath, isDirectory: initialPathIsDirectory }],
      };
      if (
        this.fileOperationEvents &&
        (await this.fileOperationEvents.will("willRename", operation)) === false
      )
        return false;
      ...
      this.emitter.emit("entry-moved", { initialPath, newPath });
      if (this.fileOperationEvents) await this.fileOperationEvents.did("didRename", operation);
      await this.revealChangedPath(newPath);
      let repo = repoForPath(newPath);
      if (repo) {
        repo.scheduleStatusSnapshotRefresh();
