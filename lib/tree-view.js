...
class TreeView {
  constructor(state) {
    ...
    this.disposables.add(
      lumine.workspaceDrops.addTarget(
        ...
        {
          surface: "tree-view",
          // Keep tree drops as the local boundary, but let pane-item drags
          // reach the pane fallback. The fallback is what supplies the split
          // affordance when a tab is dragged over the tree in a dock.
    ...
    this.disposables.add(
      this.onEntryMoved(() => this.refreshSpecialRoots()),
      this.onMoveEntryFailed(() => this.refreshSpecialRoots()),
      this.onEntryDeleted(() => {
  ...
  handleEvents() {
    ...
    const commandDisposable = lumine.commands.add(this.element, {
      ...
      "tree-view:expand-item": {
        description: "Expand the selected folder.",
        didDispatch: () => this.expandDirectory(),
      },
    ...
    this.disposables.add(commandDisposable);
  ...
  revealPath(filePath, options = {}) {
    ...
    let { show, focus, center, expandTarget } = options;
    ...
    return promise.then(async () => {
      ...
      for (let pathComponent of pathComponents) {
        ...
        const isTarget = currentPath === filePath || entry.isPathEqual(filePath);
        if (entry.kind === "directory" && (expandTarget || !isTarget)) {
          expansionPromises.push(entry.expand());
        }
        if (isTarget) {
          revealedEntry = entry;
          break;
  ...
  async revealChangedPath(entryPath) {
    try {
      ...
      await this.revealPath(entryPath, { show: true, expandTarget: false });
    } catch (error) {
  ...
  expandDirectory(isRecursive = false) {
    const directory = this.selectedEntry();
  ...
  moveSelectedEntry() {
    ...
    let dialog = new MoveDialog(oldPath, {
      move: async (initialPath, newPath) => {
        try {
          operationResult = await this.runFileMove(initialPath, newPath);
      ...
      onMove: async ({ initialPath, newPath, open }) => {
        ...
        // The move transaction has already retargeted open documents.
  ...
  openSelectedEntryInNewWindow() {
    ...
    const pathsToOpen = fs.isDirectorySync(pathToOpen)
      ? [pathToOpen]
      : [...lumine.project.getPaths(), pathToOpen];
  ...
  selectAll() {
    const rows = this.visibleRows;
    const focused = rows.includes(this.lastFocusedEntry)
      ? this.lastFocusedEntry
      : (rows[0] ?? null);
    this.addProjectsView?.clearSelection();
    ...
    const selected = this.selectRange(rows[0], rows.at(-1));
    this.lastFocusedEntry = focused;
    this.showMultiSelectMenuIfNecessary();
    return selected;
  }
  ...
  async runMovePlan(plan) {
    ...
    try {
      ...
      result = await this.runFileMove(initialPath, newPath, { executionPlan });
  ...
  async runFileMove(initialPath, newPath, options = {}) {
    const planned = options.executionPlan?.renames || [
      {
        oldPath: initialPath,
        newPath,
        isDirectory: this.operationPathSnapshot(initialPath)?.isDirectory === true,
      },
    ];
    ...
    try {
      ...
      result = await this.fileOperationProcess.run("move", initialPath, newPath, options);
      return result;
    } catch (error) {
      result = error;
      throw error;
    ...
    if (entry) {
      ...
      if (initialPaths) {
        ...
        for (let j = initialPaths.length - 1; j >= 0; j -= 1) {
          // Collapse the moved subtree before rebuilding its destination rows.
