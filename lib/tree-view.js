...
class TreeView {
  constructor(state) {
    ...
    process.nextTick(() => {
      ...
      this.disposables.add(
        lumine.styles.onDidAddStyleElement(onStylesheetsChanged),
        lumine.styles.onDidRemoveStyleElement(onStylesheetsChanged),
        lumine.styles.onDidUpdateStyleElement(onStylesheetsChanged),
      );
    });
  ...
  // A repository arriving or leaving changes what the entries report, never
  // which entries there are, so it is handed to them where they stand. This is
  // deliberately not `updateRoots`: the repository registry scans each project
  // root for nested checkouts after startup and emits one event per checkout it
  // finds, and rebuilding every root for each of those threw the tree back to
  // the top — a rebuilt root loads its children asynchronously, so the content
  // collapses to a couple of rows in between and the scroller clamps the offset
  // away before they arrive.
  repositoriesChanged() {
    if (lumine.isDestroying || !lumine.project) return;

    for (const root of this.roots) root.directory.repositoryChanged();
    for (const section of this.specialRoots) {
      for (const entry of section.entries) entry.item?.repositoryChanged?.();
    }

    // Icon hints — repository root, submodule — are resolved when a row's icon
    // is applied, so a row bound before its repository was registered wears a
    // plain folder however many times it syncs; a clean repository emits no
    // status change to sync it either. Re-apply the icon on every bound view;
    // a row bound later computes fresh hints on its own.
    for (const entry of this.treeEntries) {
      for (const view of entry.views) view.refreshIcon();
    }
  }
  ...
  updateRoots(expansionStates = {}, selectionState = null) {
    ...
    // shutting down, after LumineEnvironment has already cleared its project.
    if (lumine.isDestroying || !lumine.project) return;
    ...
    return promise.then(async () => {
      ...
      for (let pathComponent of pathComponents) {
        ...
        if (currentPath === filePath || entry.isPathEqual(filePath)) {
          this.selectEntry(entry);
          this.scrollToEntry(entry, center);
          return;
        }
      }
    });
  ...
  moveSelectedEntry() {
    ...
    let dialog = new MoveDialog(oldPath, {
      ...
      onMove: ({ initialPath, newPath, open }) => {
  ...
  copySelectedEntry() {
    ...
    let dialog = new CopyDialog(oldPath, {
      ...
      onCopy: ({ initialPath, newPath, open }) => {
  ...
  add(isCreatingFile) {
    ...
    dialog.onDidCreateFile(({ path: createdPath, open }) => {
      this.treeEntryForPath(createdPath)?.reload();
      // A file the confirm did not ask to open is selected instead, the way a
      // new folder is: the point of not opening it is to stay in the tree.
      if (open) {
        lumine.workspace.open(createdPath);
      } else {
        this.selectEntryForPath(createdPath);
      }
      if (lumine.config.get("tree-view.squashDirectoryNames")) this.updateRoots();
  ...
  onStylesheetsChanged() {
    ...
    // The redraw destroys the scroll box, which takes the offset with it, so
    // put it back before anything reads the tree again. Assigning through
    // `setScrollTop` would decline — the cache still holds this value — and the
    // element is the one that lost it.
    this.scroller.scrollTop = this.scrollPosition.top;
    this.scroller.scrollLeft = this.scrollPosition.left;
    this.measureRowHeights();
    this.rebuildVisibleRows();
  }
  ...
  onDragStart(event) {
    ...
    event.dataTransfer.setData("lumine-tree-view-event", "true");
  ...
  isLumineTreeViewEvent(event) {
    for (let item of event.dataTransfer.items) {
      if (item.type === "lumine-tree-view-event" || item.kind === "file") return true;
    }
    return false;
  }
