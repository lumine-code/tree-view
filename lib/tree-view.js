...
class TreeView {
  constructor(state) {
    ...
    // Three pieces of state, not one. The cursor is the row the keyboard acts
    // on and the anchor is the row a range grows from; keeping them apart is
    // what lets shift-arrow sweep a range without losing the end it started at,
    // and what a plain arrow resets by moving both. The cursor paints itself
    // only once a key has moved it — a pointer-driven session never shows one.
    this.cursorEntry = null;
