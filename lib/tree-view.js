...
class TreeView {
  constructor(state) {
    ...
    this.disposables.add(
      ...
      lumine.config.observe("tree-view.stickyHeaders", (stickyHeaders) => {
  ...
  handleEvents() {
    ...
    lumine.commands.add(this.element, {
      ...
      "tree-view:open-in-this-window": () => this.openSelectedEntryInThisWindow(),
    ...
    for (let i = 0; i < 9; i++) {
      lumine.commands.add(this.element, `tree-view:open-selected-entry-in-pane-${i + 1}`, () =>
    ...
    this.disposables.add(
      ...
      lumine.repositories.onDidAddRepository(debounce(() => this.updateRoots(), 50)),
  ...
  // `getPreferredLocation` is only consulted when the item is opened, so
  // changing the side of an already-open tree means taking it out of the dock
  // it is in and opening it again. Passing `moved` keeps the item alive through
  // the removal; without it the pane destroys it and the tree comes back empty.
  async moveToPreferredLocation() {
    const pane = lumine.workspace.paneForItem(this);
    if (!pane) return;
    const hadFocus = this.hasFocus();
    pane.removeItem(this, true);
    await this.show(hadFocus);
  }
  ...
  copySelectedEntryPath(relativePath = false) {
    ...
    return lumine.clipboard.write(pathToCopy);
  }
  ...
  openSelectedEntryInNewWindow() {
    ...
    if (pathToOpen) {
      return lumine.app.openWindow({ pathsToOpen: [pathToOpen], newWindow: true });
    }
  }
