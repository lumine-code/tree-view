...
const FILE_PATHS_DRAG_TYPE = "application/x-lumine-file-paths";
...
class TreeView {
  ...
  onDragStart(event) {
    ...
    if (filePaths.length > 0) {
      event.dataTransfer.setData(FILE_PATHS_DRAG_TYPE, JSON.stringify(filePaths));
      event.dataTransfer.setData("text/plain", filePaths.join("\n"));
      event.dataTransfer.setData(
        "text/uri-list",
        filePaths
          .map((filePath) => (URL.canParse(filePath) ? filePath : pathToFileURL(filePath).href))
          .join("\r\n"),
      );
    }
