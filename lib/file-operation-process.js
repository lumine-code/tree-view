...
function cancelledResult(operation) {
  return operation === "copy" ? { cancelled: true, creates: [] } : { cancelled: true, renames: [] };
}

function ensureOperationChanges(error, operation) {
  const property = operation === "copy" ? "creates" : "renames";
  if (!Array.isArray(error[property])) error[property] = [];
  return error;
}
...
module.exports = class FileOperationProcess {
  ...
  run(operation, sourcePath, destinationPath, { executionPlan } = {}) {
    if (this.destroyed) {
      return Promise.reject(new Error("The file operation process has been destroyed."));
    }
    if (!this.current && this.queue.length === 0) this.queueCompleted = 0;
  ...
  cancel(jobId) {
    ...
    if (queuedIndex !== -1) {
      ...
      if (this.queue.length === 0) this.queuePaused = false;
      job.resolve(cancelledResult(job.operation));
      this.didChange();
      return true;
    }
  ...
  getQueueProgress() {
    return {
      ...
      total: this.queueCompleted + (this.current ? 1 : 0) + this.queue.length,
    };
  }

  pauseQueue() {
    if (this.destroyed || this.queuePaused || this.queue.length === 0) return false;
  ...
  resumeQueue() {
    if (this.destroyed || !this.queuePaused) return false;
    this.queuePaused = false;
    this.pump();
    this.didChange();
    return true;
  ...
  clearQueue() {
    if (this.destroyed || this.queue.length === 0) return false;
    const queuedJobs = this.queue.splice(0);
    this.queuePaused = false;
    for (const job of queuedJobs) job.resolve(cancelledResult(job.operation));
    this.pump();
    this.didChange();
    return true;
  }
  ...
  finishCurrent(error, result) {
    ...
    this.current = null;
    this.queueCompleted++;
    if (error) ensureOperationChanges(error, job.operation);
    ...
    if (this.destroyed) this.finishDestroy();
    else this.pump();
    this.didChange();
  }
  ...
  finishDestroy() {
    if (!this.destroyed || this.current || this.destroyFinished) return;
  ...
  destroy() {
    ...
    clearTimeout(this.idleShutdownTimer);
    this.idleShutdownTimer = null;
    this.destroyPromise = new Promise((resolve) => (this.resolveDestroy = resolve));

    for (const job of this.queue.splice(0)) {
      job.reject(
        ensureOperationChanges(
          new Error("The file operation was cancelled because tree-view closed."),
          job.operation,
        ),
      );
    }
    if (this.current && !this.current.cancelRequested) this.cancel(this.current.id);
    this.finishDestroy();
