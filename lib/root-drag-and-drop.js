module.exports = class RootDragAndDropHandler {
  constructor(treeView) {
    ...
    this.windowEventSubscription = lumine.window.onDidReceive(
  ...
  dispose() {
    this.windowEventSubscription.dispose();
  }
  ...
  onDragStart(e) {
    ...
    e.dataTransfer.setData("lumine-tree-view-root-event", "true");
  ...
  onDropOnOtherWindow({ targetWindowId, fromItemIndex }) {
  ...
  onDrop(e) {
    ...
    if (fromWindowId === this.getWindowId()) {
      ...
      if (remaining.join("\0") !== projectPaths.join("\0")) {
        return lumine.project.setPaths(remaining);
      }
    } else {
      projectPaths = lumine.project.getPaths();
      ...
      if (!isNaN(fromWindowId)) {
        ...
        void lumine.window.broadcast("tree-view:project-folder-dropped", {
  ...
  isLumineTreeViewEvent(e) {
    for (let item of Array.from(e.dataTransfer.items)) {
      if (item.type === "lumine-tree-view-root-event") {
        return true;
      }
    }
