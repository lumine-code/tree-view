module.exports = class AddProjectView {
  constructor() {
    ...
    this.addProjectsButton.addEventListener("click", async () => {
      const paths = await lumine.window.pickFolder();
      if (paths) lumine.project.setPaths(paths);
    });
    ...
    this.listProjectsButton.addEventListener("click", () => {
      lumine.commands.dispatch(this.element, "project-list:toggle");
    });
