...
module.exports = class Directory {
  constructor({ name, fullPath, symlink, expansionState, isRoot, ignoredNames, useSyncFS, stats }) {
    ...
    if (lumine.config.get("tree-view.squashDirectoryNames") && !this.isRoot) {
  ...
  // Hand this entry the repository it belongs to now. Entries subscribe to their
  // repository when they are built, and the registry discovers repositories
  // asynchronously — it scans the project roots for nested checkouts after
  // startup — so an entry built before its own repository was registered
  // subscribed to nothing and reported no status until something told it
  // otherwise. That something used to be a full rebuild of every root, which
  // cost the tree its scroll offset once per checkout the scan turned up.
  repositoryChanged() {
    if (this.destroyed) return;
    ...
    for (const entry of Array.from(this.entries.values())) entry.repositoryChanged();
  }
  ...
  sortEntries(combinedEntries) {
    if (lumine.config.get("tree-view.sortFoldersBeforeFiles")) {
...
function getCompareFn() {
  return lumine.config.get("tree-view.sortMethod") === "natural" ? naturalCompare : collatorCompare;
}
