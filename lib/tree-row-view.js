...
module.exports = class TreeRowView {
  ...
  // Compute the icon hints and (re)apply the icon. The repository hints are
  // resolved at application time, and the registry discovers repositories
  // asynchronously after startup — so a row bound before its directory's
  // repository was registered wears a plain folder. The tree calls this again
  // when the repository set changes; `applyTo` replaces the element's previous
  // application wholesale, so only the latest disposable is worth keeping.
  refreshIcon() {
