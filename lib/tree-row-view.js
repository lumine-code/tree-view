...
module.exports = class TreeRowView {
  ...
  bindDirectory(entry) {
    ...
    header.replaceChildren(disclosure, name);
    element.replaceChildren(indentGuides, header);
  ...
  bindFile(entry) {
    const { element, indentGuides, disclosure, name } = this;
    element.replaceChildren(indentGuides, disclosure, name);
